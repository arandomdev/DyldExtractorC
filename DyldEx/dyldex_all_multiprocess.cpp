...
struct MessageQueue {
    ...
    struct Message {
        ...
        operator LocalMessage() {
            ...
            m.currentImage =
                std::string(currentImage.begin(), currentImage.end());
...
template <class A> void server(ProgramArguments &args, Dyld::Context &dCtx) {
    ...
    struct _SharedMemoryRemover {
        _SharedMemoryRemover() {
    ...
    auto &loggerStream = activity.loggerStream();
    ...
    if constexpr (std::is_same_v<A, Utils::Arch::x86_64>)
        ...
        clientArch = std::to_string(static_cast<int>(
            ProgramArguments::ClientSpecification::Arch::arm64));
    ...
    for (unsigned int i = 0; i < args.jobs; i++) {
        ...
        clients[clientID] = {bp::child(args.programPath.string(),
                                       bp::args(clientArgs), clientGroup),
                             ""};
    }
    ...
    while (true) {
        ...
        for (auto &[clientID, clientProc] : clients) {
            if (!clientProc.process.running()) {
                ...
                if (exitCode == 0 || exitCode == 259) {
                    ...
                    loggerStream
                        << fmt::format("Client {} has unexpectedly ended with "
                                       "exit code: {} while processing {}. "
                                       "Stopping all clients.",
                                       clientID, exitCode,
                                       clients[clientID].nextImage)
                        << std::endl;
        ...
        if (auto message =
            ...
            // clients
            if (!clients.size()) {
                loggerStream << "All clients have stopped, but there were "
                                "still images left to be process. Stopping."
                             << std::endl;
                break;
            }
        }
...
template <class A> int client(ProgramArguments &args) {
    ...
    for (auto i = args.clientSpec.start; i < args.clientSpec.end;
        ...
        if (!args.disableOutput) {
            ...
            if (outFile.good()) {
                for (auto procedure : writeProcedures) {
                    ...
                    outFile.write((const char *)procedure.source,
                                  procedure.size);
                }
                outFile.close();
            } else {
                SPDLOG_LOGGER_ERROR(activity.logger,
        ...
        if (auto nextI = i + args.clientSpec.skip;
            nextI < args.clientSpec.end) {
...
int main(int argc, char const *argv[]) {
    ...
    if (args.clientSpec.inClientMode) {
        ...
        if (!args.disableOutput && !args.outputDir) {
            std::cerr << "Output directory is required for extraction"
                      << std::endl;
            return 1;
        }
        ...
        try {
            ...
            std::cerr << fmt::format("Critical error: {}", e.what())
                      << std::endl;
            return 1;
        }
    }
